use std::{env, sync::{atomic::{AtomicBool, Ordering}, mpsc, Arc}, thread};
...
async fn main() {
    ...
    .subcommand(
        ...
        .arg(
            arg!(<DIFFICULTY> "Number of leading 0").required(true)
        )
)
    .subcommand(
    ...
    match cli_result.subcommand() {
        ...
        Some(("shiny", sub_matches)) => shiny_pokemon(sub_matches.get_one::<String>("POKE").unwrap(), sub_matches.get_one::<String>("DIFFICULTY").unwrap(), &db_pool),
...
fn shiny_pokemon(name: &String, zeros: &String, db_co: &Pool<Postgres>) {
...
fn is_shiny(hash: &str, zeros: &usize) -> bool {
    hash.starts_with(&"0".repeat(*zeros))
}
